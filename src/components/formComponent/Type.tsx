...
export default function Type({ type }: typeProps) {
	return (
		<Form.Item name="type" label="Тип" required rules={[{ required: true }]}>
			<Select className="form-alcohol-type" defaultValue={type}>
				<Select.Option value={AlcoholType.WHISKEY}>
					Виски, ром, коньяк
				</Select.Option>
				<Select.Option value={AlcoholType.WINO}>
					Вино, шампанское, вермут
				</Select.Option>
				<Select.Option value={AlcoholType.GIN}>
					Джин, текила, Самбука
				</Select.Option>
